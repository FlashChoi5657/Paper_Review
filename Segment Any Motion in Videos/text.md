...
- long-range 궤적의 입력데이터는 정규화된 pixel 좌표계($u_i$, $v_i$), visibility $\rho_i$, confidence score $c_i$ 이다($i$는 time step). 마스크는 안 보이거나 낮은 confidence일 때도 pixel 좌표를 가리키는데 적용된다. 또한 Depth-Anything으로 추정된 monocular depth map d_i 를 통합한다. d_i는 3D 장면 구조를 해석하고 공간 layout과 occulusion에 대한 이해를 향상시킨다. 
